...
if __name__ == '__main__':
    ...
    df['y_true'] = y_true
    df['y_pred'] = y_pred
    df.to_csv('y_true_pred_val.csv', index=False)
    ...
    with open("./transfer_crf_model.th", 'wb') as f:
            torch.save(model.state_dict(), f)

    vocab.save_to_files("./transfer_crf_vocabulary")

    # Save y_true and y_pred
    df = pd.DataFrame()
    df['y_true'] = y_true
    df['y_pred'] = y_pred
    df.to_csv('y_true_pred_test.csv', index=False)